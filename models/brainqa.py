...
class BrainQA(BertPreTrainedModel):
    ...
    def forward(
        ...
        outputs_encoder = self.bert_enc(input_ids)
        ...
        # Pass reconstructed hidden states to BERT decoder
        outputs_decoder = self.bert_dec(
                input_ids=input_ids,
                encoder_hidden_states = hidden_states_recon
            )

        sequence_output, _ = outputs_decoder
        ...
        if start_positions is not None and end_positions is not None:
            ...
            # # Compute VQVAE loss
            vq_recon_loss = torch.mean((hidden_states_recon - last_hidden_state)**2) # Original paper divides this by variance of total training data ??
